...
|オラクル問題<br>Oracle Problem<br>2,100 sats | <a id="oracle_problem"></a>オラクル問題は、外部データをブロックチェーン上のスマートコントラクトに取り込むという未解決の課題です。通常のビットコインのトランザクションは、スマートコントラクトの一形態であり、主に公開鍵、署名、タイムスタンプ、ブロック高といったデータを使用します。これらのデータの有効性は、ブロックチェーンを通じて客観的かつ、第三者を信頼することなくトラストレスに証明できるため、信頼性が高いです。<br>しかし、スポーツの試合の点数といった外部データを、トラストレスにスマートコントラクトに組み込むことはまだできません。現在、コントラクトの両当事者や第三者が提供する情報が正当なものであることを保証する確立された方法がないからです。いくつかのプロジェクトや計画で、オラクルの信頼性が前提とされる性質を最小限にとどめようとする取り組みが見られます。|
---
